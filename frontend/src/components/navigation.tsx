...
import { Building2, UserPlus, Gauge, FileText, CreditCard, Menu, X } from "lucide-react"
          ...
          {navItems.map(item => (
            <Link
              ...
              className={cn(
                ...
                pathname === item.href ? "bg-secondary text-foreground font-medium" : "text-muted-foreground hover:bg-secondary/50 hover:text-foreground"
              )}
            >
              <item.icon className="h-4 w-4" />
              {item.label}
